 ...
 *   DESCRIPTION:
 *
 *   Physac is a small 2D physics library written in pure C. The engine uses a
 *fixed time-step thread loop to simluate physics. A physics step contains the
 *following phases: get collision information, apply dynamics, collision solving
 *and position correction. It uses a very simple struct for physic bodies with a
 *position vector to be used in any 3D rendering API.
 *
 *   CONFIGURATION:
 *
 ...
 *       The generated implementation won't include pthread library and user
 *must create a secondary thread to call PhysicsThread(). It is so important
 *that the thread where PhysicsThread() is called must not have v-sync or any
 *other CPU limitation.
 *
 ...
 *
 *   #define PHYSAC_DEBUG
 *       Traces log messages when creating and destroying physics bodies and
 *detects errors in physics calculations and reference exceptions; it is useful
 *for debug purposes
 *
 *   #define PHYSAC_MALLOC()
 *   #define PHYSAC_FREE()
 *       You can define your own malloc/free implementation replacing stdlib.h
 *malloc()/free() functions. Otherwise it will include stdlib.h and use the C
 *standard library malloc()/free() function.
 *
 *
 *   NOTE 1: Physac requires multi-threading, when InitPhysics() a second thread
 *is created to manage physics calculations. NOTE 2: Physac requires static C
 *library linkage to avoid dependency on MinGW DLL (-static -lpthread)
 *
 *   Use the following code to compile:
 *   gcc -o $(NAME_PART).exe $(FILE_NAME) -s -static -lraylib -lpthread
 *-lopengl32 -lgdi32 -lwinmm -std=c99
 *
 *   VERY THANKS TO:
...
typedef struct PhysicsBodyData {
  ...
  PhysicsShape
      shape; // Physics body shape information (type, radius, vertices, normals)
} PhysicsBodyData;
...
//----------------------------------------------------------------------------------
PHYSACDEF void InitPhysics(void); // Initializes physics values, pointers and
                                  // creates physics loop thread
PHYSACDEF void
RunPhysicsStep(void); // Run physics step, to be used if PHYSICS_NO_THREADS is
                      // set in your main loop
PHYSACDEF void
SetPhysicsTimeStep(double delta); // Sets physics fixed time step in
                                  // milliseconds. 1.666666 by default
PHYSACDEF bool
IsPhysicsEnabled(void); // Returns true if physics thread is currently enabled
PHYSACDEF void SetPhysicsGravity(float x,
                                 float y); // Sets physics global gravity force
...
CreatePhysicsBodyRectangle(Vector2 pos, float width, float height,
                           float density); // Creates a new rectangle physics
                                           // body with generic parameters
PHYSACDEF PhysicsBody CreatePhysicsBodyPolygon(
    Vector2 pos, float radius, int sides,
    float
        density); // Creates a new polygon physics body with generic parameters
PHYSACDEF void PhysicsAddForce(PhysicsBody body,
                               Vector2 force); // Adds a force to a physics body
PHYSACDEF void
PhysicsAddTorque(PhysicsBody body,
                 float amount); // Adds an angular force to a physics body
PHYSACDEF void
PhysicsShatter(PhysicsBody body, Vector2 position,
               float force); // Shatters a polygon shape physics body to little
                             // physics bodies with explosion force
PHYSACDEF int GetPhysicsBodiesCount(
    void); // Returns the current amount of created physics bodies
PHYSACDEF PhysicsBody GetPhysicsBody(
    int index); // Returns a physics body of the bodies pool at a specific index
PHYSACDEF int
GetPhysicsShapeType(int index); // Returns the physics body shape type
                                // (PHYSICS_CIRCLE or PHYSICS_POLYGON)
...
PHYSACDEF Vector2 GetPhysicsShapeVertex(
    PhysicsBody body,
    int vertex); // Returns transformed position of a body shape (body position
                 // + vertex transformed position)
PHYSACDEF void
SetPhysicsBodyRotation(PhysicsBody body,
                       float radians); // Sets physics body shape transform
                                       // based on radians parameter
PHYSACDEF void
DestroyPhysicsBody(PhysicsBody body); // Unitializes and destroy a physics body
PHYSACDEF void ClosePhysics(
    void); // Unitializes physics pointers and closes physics loop thread
...
static double deltaTime =
    1.0 / 60.0 / 10.0 *
    1000; // Delta time used for physics steps, in milliseconds
...
static unsigned int physicsBodiesCount =
    0; // Physics world current bodies counter
static PhysicsManifold
    contacts[PHYSAC_MAX_MANIFOLDS]; // Physics bodies pointers array
static unsigned int physicsManifoldsCount =
    0; // Physics world current manifolds counter

//----------------------------------------------------------------------------------
// Module Internal Functions Declaration
//----------------------------------------------------------------------------------
static int FindAvailableBodyIndex(); // Finds a valid index for a new physics
                                     // body initialization
static PolygonData
CreateRandomPolygon(float radius,
                    int sides); // Creates a random polygon shape with max
                                // vertex distance from polygon pivot
static PolygonData
CreateRectanglePolygon(Vector2 pos,
                       Vector2 size); // Creates a rectangle polygon shape based
                                      // on a min and max positions
static void *PhysicsLoop(void *arg);  // Physics loop thread function
static void PhysicsStep(void);        // Physics steps calculations (dynamics,
                                      // collisions and position corrections)
static int FindAvailableManifoldIndex(); // Finds a valid index for a new
...
static void SolvePhysicsManifold(
    PhysicsManifold manifold); // Solves a created physics manifold between two
                               // physics bodies
static void
SolveCircleToCircle(PhysicsManifold manifold); // Solves collision between two
                                               // circle shape physics bodies
static void SolveCircleToPolygon(
    PhysicsManifold manifold); // Solves collision between a circle to a polygon
                               // shape physics bodies
static void SolvePolygonToCircle(
    PhysicsManifold manifold); // Solves collision between a polygon to a circle
                               // shape physics bodies
static void SolveDifferentShapes(
    PhysicsManifold manifold, PhysicsBody bodyA,
    PhysicsBody bodyB); // Solve collision between two different types of shapes
static void SolvePolygonToPolygon(
    PhysicsManifold
        manifold); // Solves collision between two polygons shape physics bodies
static void IntegratePhysicsForces(
    PhysicsBody body); // Integrates physics forces into velocity
static void InitializePhysicsManifolds(
    PhysicsManifold
        manifold); // Initializes physics manifolds to solve collisions
static void IntegratePhysicsImpulses(
    PhysicsManifold
        manifold); // Integrates physics collisions impulses to solve collisions
...
static void CorrectPhysicsPositions(
    PhysicsManifold manifold); // Corrects physics bodies positions based on
                               // manifolds collision information
static float FindAxisLeastPenetration(
    int *faceIndex, PhysicsShape shapeA,
    PhysicsShape shapeB); // Finds polygon shapes axis least penetration
static void
FindIncidentFace(Vector2 *v0, Vector2 *v1, PhysicsShape ref, PhysicsShape inc,
                 int index); // Finds two polygon shapes incident face
static int
Clip(Vector2 normal, float clip, Vector2 *faceA,
     Vector2 *faceB); // Calculates clipping based on a normal and two faces
static bool
BiasGreaterThan(float valueA,
                float valueB); // Check if values are between bias range
...
static uint64_t
GetTimeCount(void); // Get hi-res MONOTONIC time measure in mseconds
static double GetCurrentTime(void); // Get current time measure in milliseconds

// Math functions
static Vector2
MathCross(float value,
          Vector2 vector); // Returns the cross product of a vector and a value
static float
MathCrossVector2(Vector2 v1,
                 Vector2 v2); // Returns the cross product of two vectors
static float
MathLenSqr(Vector2 vector); // Returns the len square root of a vector
static float MathDot(Vector2 v1,
                     Vector2 v2); // Returns the dot product of two vectors
static inline float
DistSqr(Vector2 v1,
        Vector2 v2); // Returns the square root of distance between two vectors
static void
MathNormalize(Vector2 *vector); // Returns the normalized values of a vector
#if defined(PHYSAC_STANDALONE)
static Vector2 Vector2Add(Vector2 v1,
                          Vector2 v2); // Returns the sum of two given vectors
static Vector2
Vector2Subtract(Vector2 v1,
                Vector2 v2); // Returns the subtract of two given vectors
#endif

static Mat2
Mat2Radians(float radians); // Creates a matrix 2x2 from a given radians value
static void
Mat2Set(Mat2 *matrix,
        float radians); // Set values from radians to a created matrix 2x2
static inline Mat2
Mat2Transpose(Mat2 matrix); // Returns the transpose of a given matrix 2x2
static inline Vector2
Mat2MultiplyVector2(Mat2 matrix,
                    Vector2 vector); // Multiplies a vector by a matrix 2x2
...
#if !defined(PHYSAC_NO_THREADS)
  // NOTE: if defined, user will need to create a thread for PhysicsThread
  // function manually Create physics thread using POSIXS thread libraries
...
PHYSACDEF bool IsPhysicsEnabled(void) { return physicsThreadEnabled; }
...
PHYSACDEF PhysicsBody CreatePhysicsBodyCircle(Vector2 pos, float radius,
  ...
  if (newId != -1) {
    ...
    newBody->inverseMass =
        ((newBody->mass != 0.0f) ? 1.0f / newBody->mass : 0.0f);
    ...
    printf("[PHYSAC] new physics body creation failed because there is any "
           "available id to use\n");
#endif
...
PHYSACDEF PhysicsBody CreatePhysicsBodyRectangle(Vector2 pos, float width,
  ...
  if (newId != -1) {
    ...
    newBody->shape.vertexData =
        CreateRectanglePolygon(pos, (Vector2){width, height});
    ...
    for (int i = 0; i < newBody->shape.vertexData.vertexCount; i++) {
      ...
      int nextIndex =
          (((i + 1) < newBody->shape.vertexData.vertexCount) ? (i + 1) : 0);
    ...
    // Translate vertices to centroid (make the centroid (0, 0) for the polygon
    // in model space) Note: this is not really necessary
    for (int i = 0; i < newBody->shape.vertexData.vertexCount; i++) {
...
PHYSACDEF PhysicsBody CreatePhysicsBodyPolygon(Vector2 pos, float radius,
                                               int sides, float density) {
  PhysicsBody newBody = (PhysicsBody)PHYSAC_MALLOC(sizeof(PhysicsBodyData));
  usedMemory += sizeof(PhysicsBodyData);
  ...
  if (newId != -1) {
    ...
    for (int i = 0; i < newBody->shape.vertexData.vertexCount; i++) {
      ...
      float intx2 = position1.x * position1.x + position2.x * position1.x +
                    position2.x * position2.x;
      float inty2 = position1.y * position1.y + position2.y * position1.y +
...
// Shatters a polygon shape physics body to little physics bodies with explosion
// force
  ...
  if (body != NULL) {
    if (body->shape.type == PHYSICS_POLYGON) {
      ...
      for (int i = 0; i < vertexData.vertexCount; i++) {
        ...
        Vector2 positionC = Mat2MultiplyVector2(
            body->shape.transform,
      ...
      if (collision) {
        ...
        for (int i = 0; i < count; i++) {
          ...
          Vector2 center = TriangleBarycenter(vertices[i], vertices[nextIndex],
                                              PHYSAC_VECTOR_ZERO);
          ...
          PhysicsBody newBody = CreatePhysicsBodyPolygon(
              center, 10, 3,
              10); // Create polygon physics body with relevant values
          ...
          for (int j = 0; j < newData.vertexCount; j++) {
            ...
            Vector2 face = Vector2Subtract(newData.positions[nextVertex],
                                           newData.positions[j]);
          ...
          for (int j = 0; j < newBody->shape.vertexData.vertexCount; j++) {
            ...
            int nextVertex =
                (((j + 1) < newBody->shape.vertexData.vertexCount) ? (j + 1)
                                                                   : 0);
          ...
          Vector2 pointB =
              Vector2Subtract(newData.positions[1], newData.positions[0]);
          pointB.x /= 2.0f;
          pointB.y /= 2.0f;
...
PHYSACDEF int GetPhysicsBodiesCount(void) { return physicsBodiesCount; }
...
// Returns transformed position of a body shape (body position + vertex
...
PHYSACDEF Vector2 GetPhysicsShapeVertex(PhysicsBody body, int vertex) {
  ...
  if (body != NULL) {
    switch (body->shape.type) {
    case PHYSICS_CIRCLE: {
      position.x = body->position.x + cosf(360.0f / PHYSAC_CIRCLE_VERTICES *
                                           vertex * PHYSAC_DEG2RAD) *
                                          body->shape.radius;
      position.y = body->position.y + sinf(360.0f / PHYSAC_CIRCLE_VERTICES *
                                           vertex * PHYSAC_DEG2RAD) *
                                          body->shape.radius;
    } break;
    case PHYSICS_POLYGON: {
      ...
      position = Vector2Add(body->position,
                            Mat2MultiplyVector2(body->shape.transform,
...
PHYSACDEF void ClosePhysics(void) {
  ...
  if (physicsBodiesCount > 0 || usedMemory != 0)
    printf("[PHYSAC] physics module closed with %i still allocated bodies "
           "[MEMORY: %i bytes]\n",
           physicsBodiesCount, usedMemory);
  else if (physicsManifoldsCount > 0 || usedMemory != 0)
    printf("[PHYSAC] physics module closed with %i still allocated manifolds "
           "[MEMORY: %i bytes]\n",
...
static PolygonData CreateRandomPolygon(float radius, int sides) {
  ...
  for (int i = 0; i < data.vertexCount; i++) {
    ...
    Vector2 face =
        Vector2Subtract(data.positions[nextIndex], data.positions[i]);
      ...
      for (int j = i + 1; j < physicsBodiesCount; j++) {
        ...
        if (bodyB != NULL) {
          ...
          if (manifold->contactsCount > 0) {
            // Create a new manifold with same information as previously solved
            // manifold and add it to the manifolds pool last slot
...
PHYSACDEF void SetPhysicsTimeStep(double delta) { deltaTime = delta; }
...
static PhysicsManifold CreatePhysicsManifold(PhysicsBody a, PhysicsBody b) {
  PhysicsManifold newManifold =
      (PhysicsManifold)PHYSAC_MALLOC(sizeof(PhysicsManifoldData));
  ...
  if (newId != -1) {
    ...
    printf("[PHYSAC] new physics manifold creation failed because there is any "
           "available id to use\n");
#endif
...
static void SolvePhysicsManifold(PhysicsManifold manifold) {
  ...
  // Update physics body grounded state if normal direction is down and grounded
  // state is not set yet in previous manifolds
...
static void SolveCircleToCircle(PhysicsManifold manifold) {
  ...
  if (distance == 0.0f) {
    ...
    manifold->normal =
        (Vector2){normal.x / distance,
                  normal.y / distance}; // Faster than using MathNormalize() due
                                        // to sqrt is already performed
    manifold->contacts[0] =
...
static void SolveDifferentShapes(PhysicsManifold manifold, PhysicsBody bodyA,
  ...
  center = Mat2MultiplyVector2(Mat2Transpose(bodyB->shape.transform),
                               Vector2Subtract(center, bodyB->position));
  ...
  int nextIndex =
      (((faceNormal + 1) < vertexData.vertexCount) ? (faceNormal + 1) : 0);
  Vector2 v2 = vertexData.positions[nextIndex];

  // Check to see if center is within polygon
  if (separation < PHYSAC_EPSILON) {
    manifold->contactsCount = 1;
    Vector2 normal = Mat2MultiplyVector2(bodyB->shape.transform,
                                         vertexData.normals[faceNormal]);
...
static void SolvePolygonToPolygon(PhysicsManifold manifold) {
  ...
  FindIncidentFace(&incidentFace[0], &incidentFace[1], refPoly, incPoly,
                   referenceIndex);
  ...
  // Clip incident face to reference face side planes (due to floating point
  // error, possible to not have required points
  if (Clip((Vector2){-sidePlaneNormal.x, -sidePlaneNormal.y}, negSide,
           &incidentFace[0], &incidentFace[1]) < 2)
    return;
...
static void IntegratePhysicsForces(PhysicsBody body) {
  ...
  if (!body->freezeOrient)
    body->angularVelocity +=
        body->torque * body->inverseInertia * (deltaTime / 2.0);
}
...
static void InitializePhysicsManifolds(PhysicsManifold manifold) {
  ...
  manifold->staticFriction =
      sqrtf(bodyA->staticFriction * bodyB->staticFriction);
  manifold->dynamicFriction =
      sqrtf(bodyA->dynamicFriction * bodyB->dynamicFriction);
  ...
  for (int i = 0; i < manifold->contactsCount; i++) {
    ...
    if (MathLenSqr(radiusV) <
        (MathLenSqr((Vector2){gravityForce.x * deltaTime / 1000,
                              gravityForce.y * deltaTime / 1000}) +
         PHYSAC_EPSILON))
...
static void IntegratePhysicsImpulses(PhysicsManifold manifold) {
  ...
  for (int i = 0; i < manifold->contactsCount; i++) {
    ...
    Vector2 impulseV = {manifold->normal.x * impulse,
    ...
    if (absImpulseTangent < impulse * manifold->staticFriction)
      tangentImpulse =
          (Vector2){tangent.x * impulseTangent, tangent.y * impulseTangent};
    else
      tangentImpulse =
          (Vector2){tangent.x * -impulse * manifold->dynamicFriction,
...
static void CorrectPhysicsPositions(PhysicsManifold manifold) {
  ...
  correction.x =
      (max(manifold->penetration - PHYSAC_PENETRATION_ALLOWANCE, 0.0f) /
...
static float FindAxisLeastPenetration(int *faceIndex, PhysicsShape shapeA,
  ...
  for (int i = 0; i < dataA.vertexCount; i++) {
    ...
    // Retrieve vertex on face from A shape, transform into B shape's model
    // space
...
static void FindIncidentFace(Vector2 *v0, Vector2 *v1, PhysicsShape ref,
                             PhysicsShape inc, int index) {
  ...
  referenceNormal =
      Mat2MultiplyVector2(ref.transform, referenceNormal); // To world space
  referenceNormal =
      Mat2MultiplyVector2(Mat2Transpose(inc.transform),
                          referenceNormal); // To incident's model space
